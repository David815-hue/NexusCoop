...
class ReporteMCuentaController extends Controller
{
    /**
    ...
    public function index()
    {
        ...
        return view('ModuloReportes.MovCuenta.reporteMovimientoCuenta')->with('ResulTodosRepMovCuenta')->with('ResulTodosRepMovCuenta', json_decode($response,true));
     ...
     */
    public function GetReporteMovCuenta($COD_REPMOVCUENTA)
    {
        $response = Http::get('http://localhost:8080/SEC_REPORTEMOVCUENTA/'.$COD_REPMOVCUENTA);
        return view('ModuloReportes.reporteMovimientoCuenta')->with('ResulReporteMovCuenta', json_decode($response,true));
    }
